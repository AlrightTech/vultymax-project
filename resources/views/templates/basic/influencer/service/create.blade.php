...
<div class="container-fluid d-flex justify-content-end p-0">
    <button class="btn btn--base py-1 px-2" type="button">
        ...
        <form action="{{ route('influencer.service.store', @$service->id) }}" method="POST"
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    ...
                    <div class="profile-thumb p-0 text-center shadow-none">
                        <div class="thumb">
                            <img id="upload-img"
                                ...
                                alt="userProfile">
                            <label class="badge badge--icon badge--fill-base badge-fill-red update-thumb-icon" for="update-photo"><i
                    ...
                    <small class="text--warning">@lang('Supported files'): @lang('jpeg'), @lang('jpg'), @lang('png').
                ...
                <div class="col-lg-8">
                    <div class="form-group">
                        ...
                        <select class="form-select form--control select2" id="category_id" name="category_id" required>
                            ...
                            <option value="{{ $category->id }}" @if ($categoryId==$category->id) selected="selected"
                                @endif>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control form--control" name="title" type="text"
                            value="@if (@$service) {{ @$service->title }}@else{{ old('title') }} @endif" required>
                    </div>
            ...
            <div class="form-group skill-body">
                ...
                <select class="form-control form--control select2-auto-tokenize" multiple="multiple" name="tags[]"
                    required>
                    @foreach (@$tags as $tag)
                    <option value="{{ @$tag->name }}" {{ in_array($tag->name,$selectedTag) ? 'selected' : '' }}>{{
                        __(@$tag->name) }}</option>
                    @endforeach
                </select>
            </div>
...
{{ old('description',@$service->description) }}
            ...
            <div class="key-point d-flex mb-3 gap-2">
                <input class="form-control form--control" name="key_points[]" type="text" value="{{ __($point) }}"
                    required>
                <button class="btn btn--danger remove-button @if ($loop->first) disabled @endif border-0"
